...
1. Check if the engine is still running on the stuck node.
    ...
    The issue likely exists if the output shows that the engine is not running.
...
1. Check if all volumes are healthy.
    ...
    All volumes must be marked `healthy`. If this is not the case, you can uncordon nodes to make volumes healthy again.
